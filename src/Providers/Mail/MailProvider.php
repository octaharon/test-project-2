...
class MailProvider implements ServiceProviderInterface {
    private $app;
    private $from = 'noreply@localhost';

    private function getTransporter() {
        return \Swift_SmtpTransport::newInstance($this->app['config']['mail']['host'], $this->app['config']['mail']['port'], $this->app['config']['mail']['secure'])
    ...
    public function sendMail($mailTo, $subject, $text, $mailFrom = null) {
        if (!$mailFrom) {
            $mailFrom = $this->app['config']['mail']['email'];
        }
